...
from data.pages.positions_7 import get_controllers, get_positions
...
for col_symbol in controllers:
    ...
    df = pd.DataFrame(
        [(p.i, p.user, *p.user_state, p.collateral_usd, p.health) for p in curr_positions],
        columns=["n", "user", "collateral", "stablecoin", "debt", "N", "collateral_usd", "health"],
    )
